...
mkdir -p "${LOG_DIR}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/check_dspark_hidden_pipeline_alignment_$(date +%Y%m%d_%H%M%S).log}"

read -r -a TARGET_LAYER_ID_ARGS <<< "${TARGET_LAYER_IDS}"

args=(
  "${REPO_ROOT}/script/debug/check_dspark_hidden_pipeline_alignment.py"
...
echo "Writing DSpark hidden pipeline alignment check to ${LOG_FILE}"
echo "Command:"
